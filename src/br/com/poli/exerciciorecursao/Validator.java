...
public class Validator {
	...
	private String verificaCpf(String[] cpfs, String cpfVerificado, int cpf1, int cpf2){ 		// metodo recursivo
		...
		if(cpfVerificado.length()== 11){		// se o cpf estiver completo (com os 11 digitos)
			...
			while(cpf1<qtd){
				while(cpf2<qtd){														// compara o valor correspondente a posicao do cpf atual com o do pr�ximo,
					if(cpfs[cpf1].charAt(pos) != cpfs[cpf2].charAt(pos)){			// se for diferente, concatena com o valor do primeiro, e chama o m�todo para o pr�ximo digito

						cpfVerificado+=cpfs[cpf1].charAt(pos);
						cpfVerificado = verificaCpf(cpfs, cpfVerificado,cpf1,cpf2);

						if(cpfVerificado.length()==11){						// se no caso base retornar 11 digitos, o cpf � v�lido
							...
							for(int count=cpf2; count<qtd; count++){
								...
								if(cpfVerificado.length()==11){							// se tiver os 11 digitos, o cpf � v�lido.
						...
						cpfVerificado+=cpfs[cpf1].charAt(pos);				// se os valores forem iguais, n�o h� necessidade de checar 2x
						cpfVerificado = verificaCpf(cpfs, cpfVerificado,cpf1,cpf2);
					}
					...
					if(cpfVerificado.length()==11){							// se tiver os 11 digitos, o cpf � v�lido.
						...
						if(cpf1==qtd-2 && cpf2==qtd-1){						// se verificou todas as combinacoes possiveis
							...
							cpf1++;											// a combinacao ser� agora do pr�ximo cpf
							cpf2=cpf1+1;									// com o pr�ximo do pr�ximo.
						}
						else{
